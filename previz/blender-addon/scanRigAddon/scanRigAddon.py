...
class SetupOperator(bpy.types.Operator):
    ...
    camDistance: bpy.props.FloatProperty(name="Camera Distance",
                                         description="Distance (in meters) from the center of the world", default=0.5,
                                         min=0.1, max=1)  # In meters
    flashDistance: bpy.props.FloatProperty(name="Flash Distance",
                                           description="Distance (in meters) from the center of the world", default=1,
                                           min=0.1, max=2)  # In meters
    ledDistance: bpy.props.FloatProperty(name="Led Distance",
                                         description="Distance (in meters) from the center of the world", default=1,
                                         min=0.1, max=2)  # In meters
    ledAngle: bpy.props.FloatProperty(name="Led Angle", description="Angle (in degrees) around the center of the world",
                                      default=37.5, min=0, max=45)  # In degrees

    nbCamera: bpy.props.IntProperty(name="Number of Cameras", description="Number of cameras on one slice", default=3,
                                    min=1, max=25, step=1)
    startAngle: bpy.props.FloatProperty(name="Starting Angle", description="Angle of start for the column of cameras",
                                        default=-30, min=-80, max=80)
    stopAngle: bpy.props.FloatProperty(name="Stopping Angle", description="Stopping angle for the column of cameras",
                                       default=30, min=-80, max=80)
    ...
    domeDistance: bpy.props.FloatProperty(name="Dome Distance",
                                          description="Distance (in meters) from the center of the world", default=1,
...
class RenderPropertyGroup(bpy.types.PropertyGroup):
    ...
    rotAngle: bpy.props.IntProperty(name="Rotation Angle",
                                    ...
                                    default=15, min=15, max=180, step=15)  # In degrees
    stepDividerPhotometry: bpy.props.IntProperty(name="Photometry Step Divider",
            ...
            for cam in camerasObjs:
                ...
                if renderMode == 'A' or renderMode == 'AP':
                    ...
                    for step in range(0, int(360 / rotAngle)):
                        origin.rotation_euler[2] = math.radians(
                            step * rotAngle)  # Rotate the origin on each step of the process
                ...
                if renderMode == 'P' or renderMode == 'AP':
                    ...
                    for step in range(0, int(360 / (rotAngle * stepDividerPhotometry))):
                        origin.rotation_euler[2] = math.radians(
                            step * rotAngle * stepDividerPhotometry)  # Rotate the origin on each step of the process
                        for light in flashLightsObjs:
